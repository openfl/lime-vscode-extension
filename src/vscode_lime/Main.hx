...
class Main {
    ...
    function new(context:ExtensionContext) {
        ...
        serverAPI = vshaxe.exports;

        if (untyped !serverAPI) trace ("vshaxe API not available?");
        ...
        if (serverAPI.isReady) {
            ...
            //trace ("onReady");
            serverAPI.onReady = onHaxeServerReady;
        }
    ...
    public function getDisplayArguments():Array<String> {
        if (displayArguments.length == 0 && serverReady) {
            updateDisplayArguments();
        }
    ...
    function onDidChangeDisplayArguments(args:Array<String>):Void {
        if (serverReady) {
            serverAPI.updateDisplayArguments(args);
        }
    }
}
